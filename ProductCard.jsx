...
export default function ProductCard({ product }) {
  ...
  function handleLeave() { if (ref.current) ref.current.style.transform = 'perspective(900px) rotateY(0) rotateX(0)'; }
  function addToCart(e) {
    e.preventDefault(); e.stopPropagation();
    const current = JSON.parse(localStorage.getItem(CART_KEY) || '[]');
    const found = current.find((item) => item.id === product.id);
    if (found) found.quantity += qty;
    else current.push({ id: product.id, name: product.name, image: product.image, price: product.price, quantity: qty });
    ...
    <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: .4 }}>
      <div style={{ borderRadius: 16, overflow: 'hidden', background: 'var(--surface-2)', border: '1px solid var(--border)', boxShadow: '0 10px 30px rgba(0,0,0,.08)' }}>
        <Link to={`/product/${product.id}`} style={{ display: 'block' }} aria-label={`عرض ${product.name}`}>
          <div ref={ref} onMouseMove={handleMove} onMouseLeave={handleLeave} style={{ position: 'relative', height: 250, background: product.image ? `center/cover no-repeat url(${product.image})` : 'var(--surface-2)', transition: 'transform .2s ease' }}>
            <span style={{ position: 'absolute', top: 10, insetInlineEnd: 10, padding: '5px 9px', borderRadius: 999, background: 'rgba(15,12,18,.78)', color: 'var(--gold)', fontSize: 10 }}>{product.category}</span>
          </div>
          <div style={{ padding: '14px 16px 6px' }}>
            <div style={{ fontSize: 14, color: 'var(--text-primary)', marginBottom: 6 }}>{product.name}</div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{product.audience} · مقاسات متنوعة</div>
