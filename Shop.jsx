  ...
  return (
    <div className="container" style={{ padding: '48px 24px 90px' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 34, flexWrap: 'wrap' }}>
        <button onClick={() => setFilter('all')} className={filter === 'all' ? 'btn-primary' : 'btn-ghost'} style={{ fontSize: 13, padding: '9px 18px' }}>الكل</button>
        {Object.entries(tierLabels).map(([key, t]) => <button key={key} onClick={() => setFilter(key)} className={filter === key ? 'btn-primary' : 'btn-ghost'} style={{ fontSize: 13, padding: '9px 18px' }}>{t.icon} {t.label}</button>)}
      </div>
