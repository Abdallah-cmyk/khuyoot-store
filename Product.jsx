  ...
  return (
    <div className="container" style={{ padding: '48px 24px 90px', display: 'grid', gridTemplateColumns: 'minmax(0,1.1fr) minmax(0,0.9fr)', gap: 48 }}>
      <div>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 10 }}>صور المنتجات الحقيقية ستُضاف عند جاهزية التصوير.</p>
      </div>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', lineHeight: 1.9 }}>قطعة مختارة ضمن فئة {tier.label}. راجع المقاس ودليل العناية قبل إتمام الطلب.</p>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8 }}>المقاس</p>
        <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>{sizes.map((s) => <button key={s} onClick={() => setSize(s)} style={{ width: 42, height: 42, borderRadius: 10, border: size === s ? '2px solid var(--gold)' : '1px solid var(--border)', background: size === s ? 'var(--gold-soft)' : 'var(--surface-2)', color: 'var(--text-primary)', fontWeight: 600 }}>{s}</button>)}</div>
        ...
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border)', fontSize: 13, color: 'var(--text-secondary)', lineHeight: 2 }}><div>الخامة: تُحدد في وصف المنتج.</div><div>الشحن: داخل مصر حسب المحافظة وشركة الشحن.</div><div>الاسترجاع: وفق سياسة الاسترجاع المنشورة في الموقع.</div></div>
      </motion.div>
    </div>
  );
}
